...
with open(fileName) as f:
    ...
    for line in f:
        ...
        if(parity(puz,int(length))):
            if(type=="B"):
                solution = str(BFS(puz))
                end = time.perf_counter()
                print("BFS found "+solution+" moves found in "+str(end-start)+" seconds")
            if(type=="I"):
                solution = str(BFS(puz))
                end = time.perf_counter()
                print("IDDFS found "+solution+" moves found in "+str(end-start)+" seconds")
            if(type == "A"):
                solution=str(AStar(puz))
                end = time.perf_counter()
                print("A* found "+solution+" moves found in "+str(end-start)+" seconds")
            if(type == "!"):
                solution1 = str(BFS(puz))
                end1=time.perf_counter()
                solution2 = str(IDDFS(puz))
                end2=time.perf_counter()
                solution3 = str(AStar(puz))
                end3=time.perf_counter()
                print("BFS found "+solution1+" moves found in "+str(end1-start)+" seconds")
                print("IDDFS found "+solution2+" moves found in "+str(end2-end1)+" seconds")
                print("A* found "+solution3+" moves found in "+str(end3-end2)+" seconds")
        else:
            end = time.perf_counter()
